...
def delete_photographer(display_name):
    mongo_delete_photographer_by_name(display_name)
    return 'NoContent', 204

   
def put_photographer(display_name, photographer):
    try:   
       print(display_name)                                                                                                                                    
       mongo_update_photographer_by_name(display_name, photographer['first_name'], photographer['last_name'], photographer['interests'])                     
       return 'Modified', 202, {'location': '/photographer/' + str(display_name)} 
    except pymongo.errors.ServerSelectionTimeoutError as sste:                  
        return 'Mongo unavailable', 503  
    except (Photographer.DoesNotExist, InvalidId) as e:
        return 'Not Found', 404
